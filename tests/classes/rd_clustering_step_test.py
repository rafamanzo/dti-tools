...
from src.classes.rd_clustering_step import RDClusteringStep
...
class RDClusteringStepTestCase(unittest.TestCase):
    def setUp(self):
        self.rd_clustering_step = RDClusteringStep()
    ...
    def test_get_cluster(self):
        ...
        self.rd_clustering_step.eps = 1
        self.rd_clustering_step.min_pts = 0
        self.rd_clustering_step.mask = mask
        self.rd_clustering_step.tensor = tensor
        self.rd_clustering_step.max_rd_difference = 0.1

        rd_dbscan_mock = Mock()

        with patch('__main__.RDDBSCAN', rd_dbscan_mock, create=True):
            self.rd_clustering_step.get_clusterer()
