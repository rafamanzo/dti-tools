...
"""Container for MapDifferenceStep class"""
...
class MapDifferenceStep(CPUParallelStep):
    """Extracts the given region from a compound mask"""

    def __init__(self):
        super(MapDifferenceStep, self).__init__()
        self.__coefficient = 1
        self.__map1 = np.zeros((0, 0, 0)) # pylint: disable=E1101
        self.__map2 = np.zeros((0, 0, 0)) # pylint: disable=E1101
    ...
    def load_data(self):
        self.__map1 = nib.load(str(sys.argv[1]))
        self.__map2 = nib.load(str(sys.argv[2]))
        self.shape = (self.__map1.shape[0],
                      self.__map1.shape[1],
                      self.__map1.shape[2])
        self.differences_map = np.zeros(self.shape, dtype=np.float32)

    def process_partition(self, x_range, y_range, z_range):
        map1_data = self.__map1.get_data()
        map2_data = self.__map2.get_data()
        ...
        for x in range(x_range[0], x_range[1]):         # pylint: disable=C0103,C0301
            for y in range(y_range[0], y_range[1]):     # pylint: disable=C0103,C0301
                for z in range(z_range[0], z_range[1]): # pylint: disable=C0103,C0301
                    self.queue.put(((x, y, z), m.fabs(map1_data[(x,y,z)] - map2_data[(x,y,z)])))

    def consume_product(self, product):
        self.differences_map[product[0]] = product[1]

    def save(self):
        map1_name = sys.argv[1].split('/')[-1].split('.')[0]
        map2_name = sys.argv[2].split('/')[-1].split('.')[0]

        extracted_mask_img = nib.Nifti1Image(self.differences_map,
                                             self.__map1.get_affine())

        extracted_mask_img.to_filename("%s_minus_%s_map.nii.gz" % (map1_name, map2_name))