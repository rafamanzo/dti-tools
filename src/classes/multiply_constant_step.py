...
"""Container for MultiplyConstantStep class"""
...
class MultiplyConstantStep(CPUParallelStep):
    """Extracts the given region from a compound mask"""

    def __init__(self):
        super(MultiplyConstantStep, self).__init__()
        self.__constant = 1.0
        self.__map = np.zeros((0, 0, 0))         # pylint: disable=E1101
        self.__multiplied = np.zeros((0, 0, 0, 0)) # pylint: disable=E1101

    def validate_args(self):
        if len(sys.argv) != 3:
            print('This program expects two arguments: '+
                  ' map file; multiplication constant.',
                  file=sys.stderr)
            exit(1)
    ...
    def load_data(self):
        ...
        self.__constant = float(sys.argv[2])
        self.shape = (self.__map.shape[0],
                      self.__map.shape[1],
                      self.__map.shape[2])
        self.__multiplied = np.zeros(self.shape, dtype=np.float32)
        ...
        for x in range(x_range[0], x_range[1]):         # pylint: disable=C0103,C0301
            for y in range(y_range[0], y_range[1]):     # pylint: disable=C0103,C0301
                for z in range(z_range[0], z_range[1]): # pylint: disable=C0103,C0301
                    self.queue.put(((x, y, z), map_data[(x, y, z)] * self.__constant))

    def consume_product(self, product):
        self.__multiplied[product[0]] = product[1]

    def save(self):
        file_prefix = sys.argv[1].split('/')[-1].split('.')[0]

        extracted_mask_img = nib.Nifti1Image(self.__multiplied,
                                             self.__map.get_affine())

        extracted_mask_img.to_filename("%s_multiplied_by_%s.nii.gz" % (file_prefix,
                                                                sys.argv[2].replace(".","")))