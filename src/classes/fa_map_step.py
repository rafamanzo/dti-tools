...
class FAMapStep(CPUParallelStep):
    ...
    def load_data(self):
        ...
        self.shape = (self.__mask.shape[0],
                      self.__mask.shape[1],
                      self.__mask.shape[2])
        self.__fa_map = np.zeros(self.shape, dtype=np.float32)
            ...
            for y in range(y_range[0], y_range[1]):     # pylint: disable=C0103,C0301
                for z in range(z_range[0], z_range[1]): # pylint: disable=C0103,C0301
                    if mask_data[(x,y,z)] == 1:
                        self.queue.put(((x, y, z), TensorIndexes(tensor_data[(x, y, z)]).fractional_anisotropy()))

    def consume_product(self, product):
        self.__fa_map[product[0]] = product[1]

    def save(self):
        extracted_mask_img = nib.Nifti1Image(self.__fa_map,
                                             self.__mask.get_affine())

        extracted_mask_img.to_filename("fa_map.nii.gz")